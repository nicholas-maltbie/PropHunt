...
namespace PropHunt.Mixed.Utilities
{
    public struct FilteringClosestHitCollector<T> : ICollector<T> where T : struct, IQueryResult
    {
        ...
        /// <summary>
        /// Gets a self filtering collider
        /// </summary>
        /// <param name="entityIndex">index of entity</param>
        /// <param name="maxFraction">maximum fraction of movement</param>
        /// <param name="collisionWorld">Collision world for the computing of entity indices</param>
        /// <returns></returns>
        public static FilteringClosestHitCollector<T> GetSelfFilteringCollector(int entityIndex, float maxFraction, CollisionWorld collisionWorld)
        {
            return new FilteringClosestHitCollector<T>(entityIndex, AllowAll, maxFraction, collisionWorld);
        }
    }
}